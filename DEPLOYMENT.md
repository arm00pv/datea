# Urgent Troubleshooting: Gunicorn Startup Error

We are seeing a persistent `ModuleNotFoundError`. To get a clear error message, we need to run the Gunicorn command directly from your terminal. Please run the following commands on your server and provide me with the **full output**.
...
3.  **Run the Gunicorn command directly:**
    This is the command from the service file. Running it here will give us a more detailed error message.
    ```bash
    /var/www/webhost/datea/venv/bin/gunicorn --workers 3 --bind 127.0.0.1:8000 inventory_management.inventory_management.wsgi:application
    ```

Please copy and paste the entire output of that final command. This will tell us exactly why the application is failing to start. Thank you for your continued patience.
